...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parsing_arg_builtins_two_bonus.c                   :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: sandriam <sandriam@student.42antananari    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/24 09:00:22 by tambinin          #+#    #+#             */
/*   Updated: 2024/12/27 09:00:59 by sandriam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ft_strncmp(tmp_cmd[0], "cd", 2) == 0)
		ft_cd(tmp_cmd, env);
	if (ft_strncmp(tmp_cmd[0], "pwd", 3) == 0)
		ft_pwd(tmp_cmd);
	if (ft_strncmp(tmp_cmd[0], "echo", 4) == 0)
		ft_echo(tmp_cmd);
	if (ft_strncmp(tmp_cmd[0], "env", 3) == 0)
		ft_env(tmp_cmd, env);
	if (ft_strncmp(tmp_cmd[0], "export", 6) == 0)
		ft_export(tmp_cmd, env);
	if (ft_strncmp(tmp_cmd[0], "unset", 5) == 0)
		ft_unset(tmp_cmd, env);
}

void	handle_exit_builtins(char **tmp_cmd, t_cmd *cmd, t_env **env)
{
	if (ft_strncmp(tmp_cmd[0], "exit", 4) == 0)
	{
