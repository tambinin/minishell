...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   executions_utils_supp_bonus.c                      :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: sandriam <sandriam@student.42antananari    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/06 11:40:16 by sandriam          #+#    #+#             */
/*   Updated: 2024/12/27 09:00:49 by sandriam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	count_command_arg(t_cmd *cmd, int *len_command)
{
	...
	while (i < cmd->len_tokens)
	{
		if (cmd->tokens[i]->type == CMD || cmd->tokens[i]->type == ARG)
		{
			...
			{
				if (cmd->tokens[i]->type == ARG
					&& ft_strlen(cmd->tokens[i]->value) == 3
					&& ft_strchr(cmd->tokens[i]->value, '*') != 0
					&& ft_strchr(cmd->tokens[i]->value, '.'))
					(*len_command)
						+= display_matching_files(cmd->tokens[i]->value);
				else
					(*len_command)++;
			}
		}
