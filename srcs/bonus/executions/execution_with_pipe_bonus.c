...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   execution_with_pipe_bonus.c                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: sandriam <sandriam@student.42antananari    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/23 10:16:09 by sandriam          #+#    #+#             */
/*   Updated: 2024/12/27 10:09:10 by sandriam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_value_arg(int *len, t_cmd *cmd, int i, int j)
{
	if (cmd->tp_arg[i][j]->type == ARG || cmd->tp_arg[i][j]->type == CMD)
	{
		...
		else
		{
			...
			if (cmd->tp_arg[i][j]->type == ARG
				&& ft_strlen(cmd->tp_arg[i][j]->value) == 3
				&& ft_strchr(cmd->tp_arg[i][j]->value, '*') != 0
				&& ft_strchr(cmd->tp_arg[i][j]->value, '.'))
				(*len)
					+= display_matching_files(cmd->tp_arg[i][j]->value);
			else
				(*len)++;
		}
	}
